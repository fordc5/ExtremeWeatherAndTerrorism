...
visualizationsAttacks <- visualizationsAttacks %>% group_by(provstate, year) %>% summarize(totalKilled = (sum(nkill))) %>% mutate(code = state.abb[match(provstate,state.name)])

source('ichoropleth.R')
ichoropleth(totalKilled ~ provstate,
  data = visualizationsAttacks,
  pal = 'PuRd',
  ncuts = 5,
  animate = 'year'
)
