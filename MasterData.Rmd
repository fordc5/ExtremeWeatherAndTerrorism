...
## Uncmment below to run and display the check.
#apply(mergedTempTerrorismData,2,function(x) sum(is.na(x)))

nnData <- mergedTempTerrorismData %>%
  select(month.x, isAttack, Region, TempDifference, AttacksPrevYearInReg, nKilledPrevYearInReg)
...
max = apply(nnData, 2, max)
min = apply(nnData, 2, min)
scaledData = as.data.frame(scale(nnData, center=min, scale=max-min))

sampleSize = 0.5 * nrow(scaledData)                                            ## Sample from 2/3 of the observations
index = sample(seq_len(nrow(scaledData)), size=sampleSize)

trainNNData <- scaledData[index,]
testNNData <- scaledData[-index,]

neuralNet <-neuralnet(isAttack~month.x+Region+TempDifference+
                      AttacksPrevYearInReg+nKilledPrevYearInReg, 
                      trainNNData, hidden=3, linear.output=TRUE)

plot(neuralNet)                                                                 ## Plot the neural network

plot.nnet(neuralNet, cex.val = 0.5)

```

Assess the neural network:
```{r}
predictTestNN <- compute(neuralNet, testNNData[c("month.x", "Region", "TempDifference", "AttacksPrevYearInReg", "nKilledPrevYearInReg")])

results <- data.frame(actual=testNNData$isAttack, prediction=predictTestNN$net.result)
results
