...
cluster <- function(latVector, longVector) {
  ...
  lat_name <- "realLat"                                         ## Name Lat column
  long_name <- "realLong"                                       ## Name Long column
  latlong <- data.frame(latVector, longVector)
  names(latlong) <- c(lat_name, long_name)
  
  latlongCluster <- kmeans(latlong, 10, nstart = 10)            ## Run k-means clustering with n centroids and nstart CV's
  
  group_lat <- "GroupLat"
  group_long <- "GroupLong"
  latlongCenterData <- data.frame(latlongCluster$centers) 
  names(latlongCenterData) <- c(group_lat, group_long)
  
  latlongCenterData$clusterCenter <- 
    apply(latlongCenterData, 1, paste, collapse = ",")
  
  group_name <- "GroupNumber"
  resultData <- data.frame(latlongCluster$cluster) 
  names(resultData) <- c(group_name)
  
  resultData <- resultData %>%
    mutate(LatLongCenter = 
             latlongCenterData$clusterCenter[GroupNumber])
  
  ## Create a new column `cluster` with 
  ## the three columns collapsed together
  resultData$cluster <- 
    apply(resultData, 1, paste, collapse = ",")
  
  return(resultData$cluster)
}
