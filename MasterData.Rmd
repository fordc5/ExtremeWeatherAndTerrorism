...
mergedTempTerrorismData <- mergedTempTerrorismData %>%
  ...
  mutate(AttacksInYearInReg = sum(isAttack)) %>%                                                 ## Sum the number of attacks in region in year
  group_by(Region) %>%
  mutate(AvgAttacksPerYearInReg = mean(AttacksInYearInReg)) %>%
  ungroup(Region, year.x)

mergedTempTerrorismData <- mergedTempTerrorismData %>%
  group_by(Region) %>%
  mutate(AttacksPrevYearInReg = lag(AttacksInYearInReg, n=30, order_by = year.x, default=AvgAttacksPerYearInReg))

## Uncomment to view head, dimension, and random sampling of cleaned mergedTempTerrorismData
head(mergedTempTerrorismData,200)
dim(mergedTempTerrorismData)
mergedTempTerrorismData[sample(nrow(mergedTempTerrorismData), 100), ]
...
Neural network data frame cleaning:
```{r}
